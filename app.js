...
function switchTab(tabId) {
    ...
    document.querySelectorAll('nav a').forEach(el => {
        el.classList.remove('bg-gray-900', 'text-white', 'border-l-2', 'border-teal-400');
        el.classList.add('text-gray-500', 'hover:text-white', 'hover:bg-gray-900');
    });
    
    const activeNav = document.getElementById(`nav-${tabId}`);
    if (activeNav) {
        activeNav.classList.remove('text-gray-500', 'hover:text-white', 'hover:bg-gray-900');
        activeNav.classList.add('bg-gray-900', 'text-white', 'border-l-2', 'border-teal-400');
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof Chart === 'undefined' || typeof Papa === 'undefined') {
        ...
        Chart.defaults.plugins.tooltip.backgroundColor = '#030712'; // gray-950
        Chart.defaults.plugins.tooltip.titleColor = '#f3f4f6'; // gray-100
        Chart.defaults.plugins.tooltip.bodyColor = '#9ca3af'; // gray-400
        Chart.defaults.plugins.tooltip.borderColor = '#1f2937'; // gray-800
...
function renderVolumeChart() {
    ...
    gradient.addColorStop(0, 'rgba(45, 212, 191, 0.2)'); // Teal 400
...
function renderMuscleSplitChart() {
    ...
    const colors = ['#2dd4bf', '#ffffff', '#9ca3af', '#4b5563', '#374151', '#1f2937', '#6b7280', '#d1d5db'];
...
function renderIntensityChart() {
    ...
    const gradient = ctx.createLinearGradient(0, 0, 0, 400);
    gradient.addColorStop(0, 'rgba(45, 212, 191, 0.5)'); // Teal 400
    gradient.addColorStop(1, 'rgba(45, 212, 191, 0.0)');
...
function renderHeatmap() {
    ...
    for (let d = new Date(oneYearAgo); d <= today; d.setDate(d.getDate() + 1)) {
        ...
        cell.className = `w-3 h-3 rounded-sm transition-all hover:scale-125 ${hasWorkout ? 'bg-teal-400 shadow-lg shadow-teal-400/20' : 'bg-gray-800'}`;
        cell.title = dateStr;
...
function renderMuscles() {
    ...
    appState.charts.pie = new Chart(ctxPie, {
        ...
        data: {
            ...
            datasets: [{
                data: sets,
                backgroundColor: [
                    '#2dd4bf', '#ffffff', '#9ca3af', '#4b5563', '#374151', '#1f2937', '#6b7280', '#d1d5db'
                ],
                borderWidth: 0
            }]
        },
    ...
    muscles.sort((a,b) => appState.muscleStats[b].sets - appState.muscleStats[a].sets).forEach(m => {
        ...
        tr.innerHTML = `
            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-200">${m}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">${formatNumber(stat.sets)}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">${formatNumber(stat.volume)} kg</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">${stat.workouts.size}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">${((stat.sets / totalSets) * 100).toFixed(1)}%</td>
        `;
...
function renderExercises() {
    ...
    filtered.forEach(ex => {
        const tr = document.createElement('tr');
        tr.className = 'hover:bg-gray-900 transition-colors border-b border-gray-800 last:border-0';
        tr.innerHTML = `
            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-200">${ex.name}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400"><span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded bg-gray-900 text-gray-300 border border-gray-800">${ex.muscle}</span></td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400 text-right">${formatNumber(ex.sets)}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400 text-right">${ex.maxWeight} kg</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400 text-right">${ex.est1RM.toFixed(1)} kg</td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button onclick="showExerciseDetails('${ex.name.replace(/'/g, "\\'")}')" class="text-teal-400 hover:text-teal-300 transition-colors font-bold uppercase text-xs tracking-wider">Analyze</button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
function renderRecords() {
    ...
    records.forEach(rec => {
        ...
        card.className = 'bg-gray-900 rounded-xl border border-gray-800 p-6 hover:border-teal-400 transition-all duration-300 group';
        card.innerHTML = `
            <div class="flex justify-between items-start mb-4">
                <div>
                    <h3 class="font-bold text-lg text-white group-hover:text-teal-400 transition-colors">${rec.name}</h3>
                    <span class="text-xs font-bold px-2 py-1 bg-gray-950 rounded text-gray-500 border border-gray-800 mt-1 inline-block uppercase tracking-wider">${rec.muscle}</span>
                </div>
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center p-2 rounded hover:bg-gray-950 transition-colors">
                    <span class="text-sm text-gray-500 uppercase tracking-wider font-bold">Max Weight</span>
                    <span class="text-lg font-black text-white italic">${rec.maxWeight} kg</span>
                </div>
                <div class="flex justify-between items-center p-2 rounded hover:bg-gray-950 transition-colors">
                    <span class="text-sm text-gray-500 uppercase tracking-wider font-bold">Est. 1RM</span>
                    <span class="text-lg font-black text-teal-400 italic">${rec.est1RM.toFixed(1)} kg</span>
                </div>
                <div class="flex justify-between items-center p-2 rounded hover:bg-gray-950 transition-colors">
                    <span class="text-sm text-gray-500 uppercase tracking-wider font-bold">Total Sets</span>
                    <span class="text-lg font-bold text-gray-400">${rec.sets}</span>
                </div>
            </div>
            <button onclick="showExerciseDetails('${rec.name.replace(/'/g, "\\'")}')" class="w-full mt-4 py-3 text-sm font-bold text-black bg-teal-400 rounded-lg hover:bg-teal-300 transition-all uppercase tracking-wide">
                View Progress
            </button>
        `;
...
function renderHistory() {
    ...
    appState.workouts.forEach(w => {
        const tr = document.createElement('tr');
        tr.className = 'hover:bg-gray-900 cursor-pointer transition-colors border-b border-gray-800 last:border-0';
        ...
        tr.innerHTML = `
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400 font-mono">${w.date.toLocaleDateString()}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-white">${w.title}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">${durationStr}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">${formatNumber(w.volume)} kg</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">${w.sets.length}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 truncate max-w-xs">${Array.from(w.exercises).slice(0,3).join(', ')}${w.exercises.size > 3 ? '...' : ''}</td>
        `;
        tbody.appendChild(tr);
    });
}
...
function openWorkoutModal(workout) {
    ...
    for (const [name, sets] of Object.entries(grouped)) {
        html += `
            <div class="border border-gray-800 rounded-xl p-4 bg-gray-900">
                <div class="flex justify-between items-center mb-3">
                    <h4 class="font-bold text-lg text-white">${name}</h4>
                    <span class="text-xs font-bold px-2 py-1 bg-gray-950 rounded border border-gray-800 text-gray-400 uppercase tracking-wider">${sets[0].muscle}</span>
                </div>
                <div class="grid grid-cols-6 gap-4 text-xs font-bold text-gray-500 mb-2 border-b border-gray-800 pb-2 uppercase tracking-wider">
                    <div>SET</div>
                    <div>KG</div>
                    <div>REPS</div>
                    <div>RPE</div>
        ...
        sets.forEach((s, i) => {
            ...
            const prBadge = s.isPR ? '<span class="ml-2 text-[10px] bg-teal-400/20 text-teal-400 px-1.5 py-0.5 rounded border border-teal-400/30 font-bold">PR</span>' : '';
            const rpeVal = s.rpe ? s.rpe : '-';
            const notes = s.notes ? `<span class="text-xs text-gray-500 italic">${s.notes}</span>` : '-';
            
            html += `
                <div class="grid grid-cols-6 gap-4 text-sm text-gray-300 items-center hover:bg-gray-950 p-1 rounded transition-colors">
                    <div class="font-medium text-gray-500">${i + 1}</div>
                    <div class="font-bold flex items-center text-white">${s.weight} ${prBadge}</div>
                    <div class="font-medium">${s.reps}</div>
                    <div class="text-gray-400">${rpeVal}</div>
                    <div class="text-gray-500">${e1rm.toFixed(1)}</div>
...
function showExerciseDetails(exerciseName) {
    ...
    els.modalContent.innerHTML = `
        <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="p-4 bg-gray-900 border border-gray-800 rounded-xl text-center">
                <div class="text-xs text-gray-500 uppercase tracking-wider mb-1">Max Weight</div>
                <div class="text-2xl font-black text-white italic">${stats.maxWeight} kg</div>
            </div>
            <div class="p-4 bg-gray-900 border border-gray-800 rounded-xl text-center">
                <div class="text-xs text-gray-500 uppercase tracking-wider mb-1">Best Est. 1RM</div>
                <div class="text-2xl font-black text-teal-400 italic">${stats.est1RM.toFixed(1)} kg</div>
            </div>
            <div class="p-4 bg-gray-900 border border-gray-800 rounded-xl text-center">
                <div class="text-xs text-gray-500 uppercase tracking-wider mb-1">Total Sets</div>
                <div class="text-2xl font-black text-gray-400 italic">${stats.sets}</div>
            </div>
        </div>
...
function renderStrengthProgression() {
    ...
    const datasets = [];
    const colors = { 'Squat': '#2dd4bf', 'Bench Press': '#ffffff', 'Deadlift': '#9ca3af' }; // Teal, White, Gray
            ...
            datasets: [
                {
                    ...
                    backgroundColor: 'rgba(45, 212, 191, 0.2)', // Teal
                    borderColor: '#2dd4bf',
                    pointBackgroundColor: '#2dd4bf'
                },
                {
...
function updateComparisonCard(period, current, last) {
    ...
    metrics.forEach(m => {
        ...
        const sign = diff > 0 ? '+' : '';
        const color = diff >= 0 ? 'text-teal-400' : 'text-red-500';
        const icon = diff >= 0 ? '↑' : '↓';
...
function renderSessionTrends() {
    ...
    gradientVol.addColorStop(0, 'rgba(45, 212, 191, 0.5)'); // Teal 400
    gradientVol.addColorStop(1, 'rgba(45, 212, 191, 0.0)');
    ...
    appState.charts.dayOfWeek = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: counts,
                backgroundColor: [
                    '#1f2937', '#2dd4bf', '#2dd4bf', '#2dd4bf', '#2dd4bf', '#2dd4bf', '#1f2937'
                ],
                borderRadius: 6
            }]
        },
    ...
    appState.charts.repRange = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#2dd4bf', '#ffffff', '#4b5563'], // Teal, White, Gray-600
                borderWidth: 0
            }]
        },
        options: {
    ...
    appState.charts.setType = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#2dd4bf', '#ffffff', '#9ca3af', '#4b5563'],
                borderWidth: 0
            }]
        },
        options: {
...
async function uploadCSVToFirebase(file) {
    ...
    try {
        ...
        if (updateEl) {
            ...
            updateEl.classList.add('text-gray-400');
            
            setTimeout(() => {
                updateEl.textContent = "Cloud Sync Complete ✓";
                updateEl.classList.remove('text-gray-400');
                updateEl.classList.add('text-teal-400');
                setTimeout(() => {
                    updateEl.textContent = originalText;
                    updateEl.classList.remove('text-teal-400');
                }, 3000);
            }, 1000);
        }
...
function renderDietView() {
    ...
    if (container) {
        ...
        DIET_CONFIG.forEach(item => {
            ...
            card.className = `p-4 rounded-xl border transition-all cursor-pointer ${isCompleted ? 'bg-teal-400 border-teal-500' : 'bg-gray-900 border-gray-800 hover:border-gray-600'}`;
            ...
            if (item.isGroup) {
                ...
                item.items.forEach(sub => {
                    ...
                    html += `<button onclick="toggleDietItem('${sub.id}', '${item.id}', true)" class="px-2 py-1 text-xs rounded border ${isSelected ? 'bg-gray-950 text-white border-gray-950' : (isCompleted ? 'bg-white/50 text-black border-black/10' : 'bg-gray-950 text-gray-400 border-gray-700 hover:border-gray-500')}">${sub.name}</button>`;
                });
                html += `</div>`;
            } else if (item.isVariable) {
                html += `<div class="font-bold ${isCompleted ? 'text-black' : 'text-white'} mb-2">${item.name}</div>`;
                ...
                item.options.forEach(opt => {
                    ...
                    html += `<button onclick="toggleDietItem('${opt.id}', '${item.id}', true)" class="px-2 py-1 text-xs rounded border ${isSelected ? 'bg-gray-950 text-white border-gray-950' : (isCompleted ? 'bg-white/50 text-black border-black/10' : 'bg-gray-950 text-gray-400 border-gray-700 hover:border-gray-500')}">${opt.label}</button>`;
                });
                html += `</div>`;
            } else {
                html += `
                    <div class="flex justify-between items-center h-full" onclick="toggleDietItem('${item.id}')">
                        ...
                        <div class="w-6 h-6 rounded-full border-2 flex items-center justify-center ${isCompleted ? 'border-gray-950 bg-gray-950 text-teal-400' : 'border-gray-600'}">
